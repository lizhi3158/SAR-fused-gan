...
def combine(src, src_path):
    if a.b_dir and a.c_dir is None:
        raise Exception("missing b_dir and c_dir")

    # find corresponding file in b_dir, could have a different extension
    basename, _ = os.path.splitext(os.path.basename(src_path))
    for ext in [".png", ".jpg"]:
        sibling_path = os.path.join(a.b_dir, basename + ext)
        c_path = os.path.join(a.c_dir, basename + ext)
        if os.path.exists(sibling_path):
            sibling = im.load(sibling_path)
            c = im.load(c_path)
            break
    ...
    if height != c.shape[0] or width != c.shape[1]:
        raise Exception("differing sizes")
    ...
    return np.concatenate([src, sibling, c], axis=1)
